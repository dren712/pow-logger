...
import type { AgentReceipt, VerificationResult } from '../lib/agent/types'
...
export default function AgentProofConsole({
  ...
  const [selectedEventIndex, setSelectedEventIndex] = useState<number>(3) // Default to git.commit
  ...
  // Toggle Tampering Attack
  const handleToggleTamper = async () => {
    setIsVerifying(true)
    if (!isTampered) {
      // Create a hostile copy where Event #3 (or git.commit) payload is modified
      const tamperedEvents = receipt.events.map(ev => {
  ...
  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#060709', color: '#f0f3f8', padding: '32px 24px', fontFamily: 'var(--font-geist-mono, monospace)' }}>
      <div style={{ maxWidth: '1280px', margin: '0 auto' }}>
        ...
        {/* Top Header Card */}
        <div style={{ backgroundColor: '#0d0f14', border: `1px solid ${verification.verified ? '#1a1e28' : '#ff4444'}`, borderRadius: '12px', padding: '24px', marginBottom: '24px', position: 'relative', overflow: 'hidden' }}>
          {/* Subtle Glow */}
          <div style={{ position: 'absolute', top: 0, right: 0, width: '300px', height: '100%', background: verification.verified ? 'radial-gradient(circle, rgba(0,255,136,0.06) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(255,68,68,0.12) 0%, transparent 70%)', pointerEvents: 'none' }} />
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'flex-start', gap: '20px' }}>
            ...
            <div style={{ textAlign: 'right' }}>
              <div style={{ 
                ...
                padding: '8px 16px', 
                borderRadius: '8px', 
                backgroundColor: verification.verified ? 'rgba(0, 255, 136, 0.1)' : 'rgba(255, 68, 68, 0.15)',
                border: `1px solid ${verification.verified ? '#00ff88' : '#ff4444'}`,
                color: verification.verified ? '#00ff88' : '#ff4444',
                fontWeight: 700,
                fontSize: '14px',
                letterSpacing: '0.5px'
              }}>
                <span style={{ fontSize: '16px' }}>{verification.verified ? '✓' : '🚨'}</span>
                <span>{verification.verified ? 'PROVENANCE VERIFIED' : 'INTEGRITY FAILURE'}</span>
              </div>
              <div style={{ fontSize: '11px', color: '#64748b', marginTop: '6px' }}>
                {receipt.events.length} Actions Verified • {receipt.batch ? '1 Sealed Batch' : '3 Batches'}
              </div>
            </div>
          </div>
        ...
        {/* THE KILLER DEMO BANNER: Database vs Public Commitment */}
        <div style={{ 
          backgroundColor: isTampered ? 'rgba(255, 68, 68, 0.08)' : '#0d0f14', 
          border: `1px solid ${isTampered ? '#ff4444' : '#1e2430'}`, 
          borderRadius: '12px', 
          padding: '20px 24px', 
          marginBottom: '24px' 
        }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', alignItems: 'center', gap: '16px' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                ...
                <span style={{ fontSize: '14px', fontWeight: 700, color: isTampered ? '#ff4444' : '#00e5ff' }}>
                  {isTampered ? 'ACTIVE DATABASE TAMPER SIMULATION DETECTED' : 'INTERACTIVE ZERO-TRUST TAMPER DEMO'}
                </span>
              </div>
              <p style={{ margin: '6px 0 0 0', fontSize: '12px', color: '#94a3b8', maxWidth: '780px' }}>
                Simulate a compromised PostgreSQL database where an attacker alters an event (e.g. replacing a git commit hash with <code style={{ color: '#ffb800' }}>fake999</code>). The verifier independently recomputes the Merkle root and catches the divergence against Solana.
              </p>
            </div>
            ...
            <button
              ...
              style={{
                backgroundColor: isTampered ? '#131720' : '#ff4444',
                color: isTampered ? '#00ff88' : '#ffffff',
                border: `1px solid ${isTampered ? '#00ff88' : '#ff4444'}`,
                padding: '10px 18px',
                borderRadius: '6px',
                fontWeight: 700,
                fontSize: '13px',
            ...
            >
              {isTampered ? '↺ Reset to Genuine Record' : '🚨 Simulate Database Intrusion'}
            </button>
          </div>

          {/* Tamper Comparison Callout */}
          {isTampered && (
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      border: `1px solid ${isTamperedEvent ? '#ff4444' : isSelected ? '#00ff88' : '#1e2430'}`,
                      cursor: 'pointer',
                      display: 'flex',
                      alignItems: 'center',
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                      ...
                      <span style={{ fontSize: '12px', color: isTamperedEvent ? '#ff8888' : '#cbd5e1' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                      <span style={{ fontSize: '11px', color: isTamperedEvent ? '#ff4444' : '#00ff88' }}>
                        {isTamperedEvent ? '✗ TAMPERED' : '✓'}
                      </span>
                    </div>
                  </div>
                )
