...
exports.SKILL_BADGES = exports.STREAK_MILESTONES = exports.BUILDER_LEVELS = void 0;
 ...
 */
function calculateStreak(createdAts) {
 ...
 */
function calculateLongestStreak(createdAts) {
    if (!createdAts || createdAts.length === 0)
        return 0;
    const logDates = [
        ...new Set(createdAts.map((d) => new Date(d).toDateString())),
...
exports.SKILL_BADGES = [
    {
        ...
        checkUnlocked: (logs) => logs.filter((l) => (l.skills || []).some((s) => s.toLowerCase().includes('anchor') || s.toLowerCase().includes('rust') || s.toLowerCase().includes('solana'))).length >= 3,
    },
    {
        id: 'security_auditor',
