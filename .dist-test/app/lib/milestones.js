"use strict";
/**
 * PROVN Protocol — Builder Level & Milestone System
 *
 * Dual-Tier Badge Architecture:
 * • Tier 1: Dynamic Evolving Builder Level (1 per builder, auto-levels up)
 * • Tier 2: Streak Milestone Trophies (earned at 7, 30, 100 day streaks)
 *
 * This module is the single source of truth for all badge/level calculations.
 * It is designed to be consumed by both client (UI) and server (API) code.
 *
 * Future: When Merkle Tree & Helius RPC are configured, these levels/milestones
 * will trigger on-chain cNFT minting via app/lib/cnft.ts.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.SKILL_BADGES = exports.PROTOCOL_TIMEZONE = exports.STREAK_MILESTONES = exports.BUILDER_LEVELS = void 0;
exports.getBuilderLevel = getBuilderLevel;
exports.getNextLevel = getNextLevel;
exports.getLevelProgress = getLevelProgress;
exports.getEarnedMilestones = getEarnedMilestones;
exports.getNextMilestone = getNextMilestone;
exports.checkNewMilestoneReached = checkNewMilestoneReached;
exports.toLocalDateString = toLocalDateString;
exports.getProtocolStartOfDay = getProtocolStartOfDay;
exports.fetchAllWalletLogs = fetchAllWalletLogs;
exports.toUTCDay = toUTCDay;
exports.toUTCDayString = toUTCDayString;
exports.calculateStreak = calculateStreak;
exports.calculateLongestStreak = calculateLongestStreak;
exports.getEarnedSkillBadges = getEarnedSkillBadges;
exports.computeBadgeSummary = computeBadgeSummary;
exports.BUILDER_LEVELS = [
    { level: 1, title: 'Apprentice Builder', emoji: '🔧', color: '#888888', minLogs: 0, glow: 'rgba(136,136,136,0.15)' },
    { level: 2, title: 'Verified Craftsman', emoji: '⚒️', color: '#00e5ff', minLogs: 7, glow: 'rgba(0,229,255,0.15)' },
    { level: 3, title: 'Senior Architect', emoji: '🏗️', color: '#ffb800', minLogs: 30, glow: 'rgba(255,184,0,0.15)' },
    { level: 4, title: 'Protocol Master', emoji: '💎', color: '#ff00ff', minLogs: 100, glow: 'rgba(255,0,255,0.15)' },
    { level: 5, title: 'Grand Legend', emoji: '👑', color: '#ff4400', minLogs: 365, glow: 'rgba(255,68,0,0.2)' },
];
function getBuilderLevel(totalLogs) {
    let current = exports.BUILDER_LEVELS[0];
    for (const level of exports.BUILDER_LEVELS) {
        if (totalLogs >= level.minLogs) {
            current = level;
        }
        else {
            break;
        }
    }
    return current;
}
function getNextLevel(totalLogs) {
    const currentLevel = getBuilderLevel(totalLogs);
    const nextIdx = exports.BUILDER_LEVELS.findIndex((l) => l.level === currentLevel.level) + 1;
    if (nextIdx >= exports.BUILDER_LEVELS.length)
        return null;
    const next = exports.BUILDER_LEVELS[nextIdx];
    return { next, logsRemaining: next.minLogs - totalLogs };
}
function getLevelProgress(totalLogs) {
    const current = getBuilderLevel(totalLogs);
    const nextInfo = getNextLevel(totalLogs);
    if (!nextInfo)
        return 100; // Max level reached
    const rangeTotal = nextInfo.next.minLogs - current.minLogs;
    const rangeProgress = totalLogs - current.minLogs;
    return Math.min(100, Math.round((rangeProgress / rangeTotal) * 100));
}
exports.STREAK_MILESTONES = [
    { days: 7, title: '7-Day Streak', emoji: '🔥', color: '#ff6600', description: 'Logged work for 7 consecutive days', mintable: true },
    { days: 14, title: '14-Day Streak', emoji: '⚡', color: '#ffb800', description: 'Logged work for 14 consecutive days', mintable: true },
    { days: 30, title: '30-Day Ironclad', emoji: '🛡️', color: '#00e5ff', description: 'Logged work for 30 consecutive days', mintable: true },
    { days: 60, title: '60-Day Titan', emoji: '⚔️', color: '#ab9ff2', description: 'Logged work for 60 consecutive days', mintable: true },
    { days: 100, title: '100-Day Legend', emoji: '💎', color: '#ff00ff', description: 'Logged work for 100 consecutive days', mintable: true },
    { days: 365, title: '365-Day Eternal', emoji: '👑', color: '#ff4400', description: 'Logged work for 365 consecutive days', mintable: true },
];
function getEarnedMilestones(currentStreak, longestStreak) {
    const bestStreak = Math.max(currentStreak, longestStreak);
    return exports.STREAK_MILESTONES.filter((m) => bestStreak >= m.days);
}
function getNextMilestone(currentStreak) {
    for (const m of exports.STREAK_MILESTONES) {
        if (currentStreak < m.days) {
            return { milestone: m, daysRemaining: m.days - currentStreak };
        }
    }
    return null; // All milestones achieved
}
/**
 * Check if a new milestone was JUST reached (for triggering mint/notification).
 * Call after incrementing streak.
 */
function checkNewMilestoneReached(previousStreak, newStreak) {
    for (const m of exports.STREAK_MILESTONES) {
        if (previousStreak < m.days && newStreak >= m.days) {
            return m;
        }
    }
    return null;
}
exports.PROTOCOL_TIMEZONE = 'Asia/Kolkata';
/**
 * Formats a Date or ISO timestamp into YYYY-MM-DD string according to a specific timezone.
 * Defaults to PROTOCOL_TIMEZONE ('Asia/Kolkata' / IST, UTC+5:30).
 */
function toLocalDateString(dateInput, timeZone = exports.PROTOCOL_TIMEZONE) {
    const d = new Date(dateInput);
    if (isNaN(d.getTime()))
        return '1970-01-01';
    try {
        const formatter = new Intl.DateTimeFormat('en-CA', {
            timeZone: timeZone || exports.PROTOCOL_TIMEZONE,
            year: 'numeric',
            month: '2-digit',
            day: '2-digit',
        });
        return formatter.format(d);
    }
    catch {
        return d.toISOString().split('T')[0];
    }
}
/**
 * Calculates the start-of-day (00:00:00.000) Date object in the canonical protocol timezone (Asia/Kolkata / IST, +05:30).
 */
function getProtocolStartOfDay(dateInput = new Date()) {
    const dateStr = toLocalDateString(dateInput, exports.PROTOCOL_TIMEZONE);
    return new Date(`${dateStr}T00:00:00.000+05:30`);
}
/**
 * Helper to fetch ALL logs for a given wallet address from Supabase, paging past default 1,000 row limits.
 * Guarantees a fail-closed contract: throws on any query failure rather than returning partial results.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
async function fetchAllWalletLogs(supabaseClient, walletAddress) {
    const PAGE_SIZE = 1000;
    let allLogs = [];
    let from = 0;
    let to = PAGE_SIZE - 1;
    let fetchMore = true;
    while (fetchMore) {
        const { data, error } = await supabaseClient
            .from('logs')
            .select('*')
            .eq('wallet_address', walletAddress)
            .order('created_at', { ascending: false })
            .order('id', { ascending: false })
            .range(from, to);
        if (error) {
            throw new Error(`Failed to fetch wallet logs at offset ${from}: ${error.message}`);
        }
        if (!data) {
            throw new Error(`Failed to fetch wallet logs at offset ${from}: Empty response data`);
        }
        if (data.length === 0) {
            fetchMore = false;
            break;
        }
        allLogs = allLogs.concat(data);
        if (data.length < PAGE_SIZE) {
            fetchMore = false;
        }
        else {
            from += PAGE_SIZE;
            to += PAGE_SIZE;
        }
    }
    return allLogs;
}
/**
 * Normalizes a Date or ISO string to a UTC Date object at midnight UTC (00:00:00.000).
 */
function toUTCDay(dateInput) {
    const d = new Date(dateInput);
    if (isNaN(d.getTime()))
        return new Date(0);
    return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()));
}
/**
 * Returns YYYY-MM-DD string representation of a Date in UTC.
 */
function toUTCDayString(dateInput) {
    const d = new Date(dateInput);
    if (isNaN(d.getTime()))
        return '1970-01-01';
    return d.toISOString().split('T')[0];
}
/**
 * Calculates current active streak from an array of ISO date strings or Date objects.
 * Single source of truth for streak calculation, evaluating day boundaries in PROTOCOL_TIMEZONE ('Asia/Kolkata' / IST).
 */
function calculateStreak(createdAts, timeZone = exports.PROTOCOL_TIMEZONE) {
    if (!createdAts || createdAts.length === 0)
        return 0;
    const tz = timeZone || exports.PROTOCOL_TIMEZONE;
    const uniqueDays = Array.from(new Set(createdAts
        .map((d) => toLocalDateString(d, tz))
        .filter((s) => s !== '1970-01-01'))).sort().reverse();
    if (uniqueDays.length === 0)
        return 0;
    const todayStr = toLocalDateString(new Date(), tz);
    const yesterdayDate = new Date();
    yesterdayDate.setDate(yesterdayDate.getDate() - 1);
    const yesterdayStr = toLocalDateString(yesterdayDate, tz);
    const mostRecent = uniqueDays[0];
    if (mostRecent !== todayStr && mostRecent !== yesterdayStr) {
        return 0;
    }
    let streak = 1;
    let checkDateStr = mostRecent;
    for (let i = 1; i < uniqueDays.length; i++) {
        const dayStr = uniqueDays[i];
        const checkDate = new Date(checkDateStr + 'T00:00:00.000Z');
        const dayDate = new Date(dayStr + 'T00:00:00.000Z');
        const diffDays = Math.round((checkDate.getTime() - dayDate.getTime()) / 86400000);
        if (diffDays === 0) {
            continue;
        }
        else if (diffDays === 1) {
            streak++;
            checkDateStr = dayStr;
        }
        else {
            break;
        }
    }
    return streak;
}
/**
 * Calculates the longest streak achieved in days for a target timezone.
 * Defaults to PROTOCOL_TIMEZONE ('Asia/Kolkata' / IST, UTC+5:30).
 */
function calculateLongestStreak(createdAts, timeZone = exports.PROTOCOL_TIMEZONE) {
    if (!createdAts || createdAts.length === 0)
        return 0;
    const tz = timeZone || exports.PROTOCOL_TIMEZONE;
    const uniqueDays = Array.from(new Set(createdAts
        .map((d) => toLocalDateString(d, tz))
        .filter((s) => s !== '1970-01-01'))).sort();
    if (uniqueDays.length === 0)
        return 0;
    let maxStreak = 1;
    let currentStreak = 1;
    for (let i = 1; i < uniqueDays.length; i++) {
        const prevStr = uniqueDays[i - 1];
        const currStr = uniqueDays[i];
        const prevDate = new Date(prevStr + 'T00:00:00.000Z');
        const currDate = new Date(currStr + 'T00:00:00.000Z');
        const diffDays = Math.round((currDate.getTime() - prevDate.getTime()) / 86400000);
        if (diffDays === 1) {
            currentStreak++;
            if (currentStreak > maxStreak)
                maxStreak = currentStreak;
        }
        else if (diffDays > 1) {
            currentStreak = 1;
        }
    }
    return maxStreak;
}
exports.SKILL_BADGES = [
    {
        id: 'rust_anchor',
        title: 'Anchor Specialist',
        emoji: '⚓',
        color: '#ff0055',
        category: 'skill',
        description: 'Logged 3+ Anchor Framework smart contract work logs',
        checkUnlocked: (logs) => logs.filter((l) => (l.skills || []).some((s) => s.toLowerCase().includes('anchor')) ||
            (l.protocols || []).some((p) => p.toLowerCase().includes('anchor'))).length >= 3,
    },
    {
        id: 'security_auditor',
        title: 'Security Specialist',
        emoji: '🛡️',
        color: '#ff4444',
        category: 'skill',
        description: 'Logged 2+ Security or Authentication focused work logs',
        checkUnlocked: (logs) => logs.filter((l) => (l.category || '').toLowerCase().includes('security') ||
            (l.skills || []).some((s) => s.toLowerCase().includes('security') || s.toLowerCase().includes('auth'))).length >= 2,
    },
    {
        id: 'open_source',
        title: 'Open Source Builder',
        emoji: '🐙',
        color: '#ab9ff2',
        category: 'quality',
        description: 'Attached 3+ GitHub-linked proof repository/PR URLs',
        checkUnlocked: (logs) => logs.filter((l) => l.github_url && l.github_url.includes('github.com')).length >= 3,
    },
    {
        id: 'permanent_archivist',
        title: 'Arweave Archivist',
        emoji: '📜',
        color: '#00e5ff',
        category: 'quality',
        description: 'Archived 5+ logs permanently to Arweave',
        checkUnlocked: (logs) => logs.filter((l) => (l.archival_state === 'receipt_obtained' || l.archival_state === 'finalized') &&
            Boolean(l.irys_tx_id) &&
            typeof l.irys_tx_id === 'string' &&
            !l.irys_tx_id.startsWith('powl_')).length >= 5,
    },
    {
        id: 'century_builder',
        title: 'Century Club',
        emoji: '💯',
        color: '#ff00ff',
        category: 'volume',
        description: 'Submitted 100+ total verified proof logs',
        checkUnlocked: (logs) => logs.length >= 100,
    },
];
function getEarnedSkillBadges(logs) {
    if (!logs || !Array.isArray(logs))
        return [];
    return exports.SKILL_BADGES.filter((b) => b.checkUnlocked(logs));
}
function computeBadgeSummary(totalLogs, currentStreak, longestStreak, logs = []) {
    return {
        level: getBuilderLevel(totalLogs),
        nextLevel: getNextLevel(totalLogs),
        levelProgress: getLevelProgress(totalLogs),
        currentStreak,
        longestStreak,
        earnedMilestones: getEarnedMilestones(currentStreak, longestStreak),
        earnedSkillBadges: getEarnedSkillBadges(logs),
        nextMilestone: getNextMilestone(currentStreak),
        totalLogs,
    };
}
