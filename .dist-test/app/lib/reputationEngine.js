...
function calculateReputation(wallet, logs) {
    const safeLogs = logs || [];
    ...
    const verifiedLogs = safeLogs.filter((l) => (0, canonicalMessage_1.verifyLogCryptographically)(l));
    ...
    const unverifiedRecords = safeLogs.filter((l) => (l.nonce || l.protocol_version === 2) && !(0, canonicalMessage_1.verifyLogCryptographically)(l)).length;
    // Archived verified proofs (Irys / Arweave confirmed)
    const archivedVerifiedProofs = verifiedLogs.filter((l) => l.archival_state === 'receipt_obtained' || l.archival_state === 'finalized' || (Boolean(l.irys_tx_id) && !l.irys_tx_id?.startsWith('powl_'))).length;
    ...
    const currentStreak = (0, milestones_1.calculateStreak)(createdAts, milestones_1.PROTOCOL_TIMEZONE);
    const longestStreak = (0, milestones_1.calculateLongestStreak)(createdAts, milestones_1.PROTOCOL_TIMEZONE);
    // Builder Level derived strictly from verified proof count
    const levelInfo = (0, milestones_1.getBuilderLevel)(totalProofs);
    const builderLevel = {
    ...
    const milestones = (0, milestones_1.getEarnedMilestones)(currentStreak, longestStreak).map((m) => m.title);
    const achievements = (0, achievements_1.evaluateAchievements)(verifiedLogs, currentStreak, longestStreak);
    ...
    const uniqueDays = new Set(createdAts
        .map((d) => (0, milestones_1.toLocalDateString)(d, milestones_1.PROTOCOL_TIMEZONE))
