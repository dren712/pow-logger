...
const canonicalMessage_1 = require("../app/lib/canonicalMessage");
const irysUploader_1 = require("../app/lib/irysUploader");
const rateLimiter_1 = require("../app/lib/rateLimiter");
const milestones_1 = require("../app/lib/milestones");
const reputationEngine_1 = require("../app/lib/reputationEngine");
const achievements_1 = require("../app/lib/achievements");
const cnftEligibility_1 = require("../app/lib/cnftEligibility");
const index_1 = require("../sdk/index");
const cardThemes_1 = require("../app/lib/cardThemes");
const githubVerifier_1 = require("../app/lib/githubVerifier");
const fs_1 = __importDefault(require("fs"));
const path_1 = __importDefault(require("path"));
try {
    const envContent = fs_1.default.readFileSync(path_1.default.join(process.cwd(), '.env.local'), 'utf-8');
...
async function runProductionTestSuite() {
    ...
    const validGithub = (0, canonicalMessage_1.validateAndNormalizeUrl)('https://github.com/dren712/pow-logger/pull/1', 'github');
    assert(validGithub === 'https://github.com/dren712/pow-logger/pull/1', 'Valid GitHub PR URL normalized correctly');
    const invalidGithubDomain = (0, canonicalMessage_1.validateAndNormalizeUrl)('https://malicious-github.com/fake/repo', 'github');
    assert(invalidGithubDomain === null, 'Non-github.com domain rejected for GitHub URL');
    const invalidScheme = (0, canonicalMessage_1.validateAndNormalizeUrl)('javascript:alert(1)', 'evidence');
    assert(invalidScheme === null, 'Non-HTTPS scheme rejected for evidence URL');
    const validEvidence = (0, canonicalMessage_1.validateAndNormalizeUrl)('https://provn-sol.vercel.app/demo', 'evidence');
    ...
    const prUrl = (0, githubVerifier_1.parseGithubUrl)('https://github.com/dren712/pow-logger/pull/1');
    assert(prUrl?.type === 'pull' && prUrl.identifier === '1', 'GitHub PR URL parsed correctly');
    const commitUrl = (0, githubVerifier_1.parseGithubUrl)('https://github.com/dren712/pow-logger/commit/7db381a');
    assert(commitUrl?.type === 'commit' && commitUrl.identifier === '7db381a', 'GitHub Commit URL parsed correctly');
    const badGithubUrl = (0, githubVerifier_1.parseGithubUrl)('https://github.com/dren712/pow-logger/issues/5');
    assert(badGithubUrl === null, 'GitHub Issue URL rejected (only PRs/Commits)');
    const spoofedDomain = (0, canonicalMessage_1.getVerifiedDomain)('evil-hacker.com');
    assert(spoofedDomain === 'provn-sol.vercel.app', 'Arbitrary host header spoofing rejected');
    const validLocalhost = (0, canonicalMessage_1.getVerifiedDomain)('localhost:3000');
    ...
    const parsedKeyBytes = (0, irysUploader_1.parseIrysPrivateKey)(dummyKeyArr);
    ...
    const currentStreakCount = (0, milestones_1.calculateStreak)(sampleDates);
    assert(currentStreakCount === 3, 'Consecutive 3-day IST streak calculated correctly');
    assert(milestones_1.PROTOCOL_TIMEZONE === 'Asia/Kolkata', 'PROTOCOL_TIMEZONE is exported as canonical Asia/Kolkata');
    const prevDayInstant = '2026-08-10T18:29:59.000Z';
    const nextDayInstant = '2026-08-10T18:30:00.000Z';
    assert((0, milestones_1.toLocalDateString)(prevDayInstant) === '2026-08-10', '18:29:59Z resolves to previous IST date 2026-08-10');
    assert((0, milestones_1.toLocalDateString)(nextDayInstant) === '2026-08-11', '18:30:00Z resolves to next IST date 2026-08-11');
    const startOfDayBound = (0, milestones_1.getProtocolStartOfDay)(nextDayInstant);
        ...
        from: () => ({
            select: () => ({
                eq: () => ({
                    order: () => ({
                        order: () => ({
                            range: (from, to) => Promise.resolve({ data: mockLogs.slice(from, to + 1), error: null })
                        })
                    })
                })
            })
    ...
    let fetchedLogsResult = [];
    let errorCaught = false;
    try {
        fetchedLogsResult = await (0, milestones_1.fetchAllWalletLogs)(mockSuccessClient, 'testWallet');
    }
    catch { }
    ...
    try {
        await (0, milestones_1.fetchAllWalletLogs)(mockErrorClient, 'testWallet');
    }
    catch (err) {
    ...
    try {
        (0, canonicalMessage_1.decodeBase58)(validNonce);
    ...
    try {
        (0, canonicalMessage_1.decodeBase58)(invalidNonceChars);
        invalidNonceParsed = true;
    }
    catch { }
    assert(invalidNonceParsed === false, 'Invalid non-Base58 nonce strictly rejected');
    const validateNonceRule = (n) => typeof n === 'string' && n.trim().length >= 8 && n === n.trim();
    ...
    const rateLimitTest1 = (0, rateLimiter_1.checkRateLimit)(testIp, 'ip', 2, 1000);
    assert(rateLimitTest1.allowed === true && rateLimitTest1.remaining === 1, 'First request within rate limit allowed');
    const rateLimitTest2 = (0, rateLimiter_1.checkRateLimit)(testIp, 'ip', 2, 1000);
    assert(rateLimitTest2.allowed === true && rateLimitTest2.remaining === 0, 'Second request reaching limit allowed');
    const rateLimitTest3 = (0, rateLimiter_1.checkRateLimit)(testIp, 'ip', 2, 1000);
    ...
    const walletAddress = bs58_1.default.encode(keypair.publicKey);
    const timestamp = new Date().toISOString();
    const nonce = 'ABCDEFGH12345678';
    const canonicalMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        walletAddress,
        timestamp,
        nonce,
    ...
    const sigBytes = tweetnacl_1.default.sign.detached(msgBytes, keypair.secretKey);
    const signature = bs58_1.default.encode(sigBytes);
    // Verify genuine signature
    const isOriginalValid = tweetnacl_1.default.sign.detached.verify(msgBytes, bs58_1.default.decode(signature), bs58_1.default.decode(walletAddress));
    ...
    const tamperedContentMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        walletAddress,
        timestamp,
        nonce,
    ...
    const isTamperedContentValid = tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(tamperedContentMsg), bs58_1.default.decode(signature), bs58_1.default.decode(walletAddress));
    assert(isTamperedContentValid === false, 'Tampered content invalidates cryptographic signature');
    // Test Tampered GitHub URL
    const tamperedGithubMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        walletAddress,
        timestamp,
        nonce,
        content: 'Built Ed25519 anti-tamper security layer for PROVN',
    ...
    const isTamperedGithubValid = tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(tamperedGithubMsg), bs58_1.default.decode(signature), bs58_1.default.decode(walletAddress));
    assert(isTamperedGithubValid === false, 'Tampered GitHub URL invalidates cryptographic signature');
    // Test Tampered Evidence URL
    const tamperedEvidenceMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        walletAddress,
        timestamp,
        nonce,
        content: 'Built Ed25519 anti-tamper security layer for PROVN',
    ...
    const isTamperedEvidenceValid = tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(tamperedEvidenceMsg), bs58_1.default.decode(signature), bs58_1.default.decode(walletAddress));
    ...
    if (supabaseUrl && anonKey && (0, canonicalMessage_1.isConfiguredSupabaseUrl)(supabaseUrl)) {
        try {
            const anonClient = (0, supabase_js_1.createClient)(supabaseUrl, anonKey);
    ...
    const canonicalRetryMsg = (0, canonicalMessage_1.buildCanonicalRetryMessage)({
        walletAddress,
        logId: retryLogId,
    ...
    const retrySigBytes = tweetnacl_1.default.sign.detached(retryBytes, keypair.secretKey);
    const retrySignature = bs58_1.default.encode(retrySigBytes);
    const isRetrySigValid = tweetnacl_1.default.sign.detached.verify(retryBytes, bs58_1.default.decode(retrySignature), bs58_1.default.decode(walletAddress));
    assert(isRetrySigValid === true, 'Authorized retry SIWS signature verifies correctly');
    // Test Retry for different wallet
    const wrongKeypair = tweetnacl_1.default.sign.keyPair();
    const wrongWalletAddress = bs58_1.default.encode(wrongKeypair.publicKey);
    const isWrongWalletValid = tweetnacl_1.default.sign.detached.verify(retryBytes, bs58_1.default.decode(retrySignature), bs58_1.default.decode(wrongWalletAddress));
    ...
    const e2eCanonicalMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: testDomain,
        walletAddress,
    ...
    const e2eSigBytes = tweetnacl_1.default.sign.detached(e2eMsgBytes, keypair.secretKey);
    const e2eSignature = bs58_1.default.encode(e2eSigBytes);
    ...
    const reconstructedMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
    ...
    const isE2EValid = tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(reconstructedMsg), bs58_1.default.decode(mockStoredRow.signature), bs58_1.default.decode(mockStoredRow.wallet_address));
    assert(isE2EValid === true, 'Persisted nonce and domain reconstruct exact signature correctly');
    // 1. Tampered Nonce
    const tamperedNonceMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: mockStoredRow.domain,
        walletAddress: mockStoredRow.wallet_address,
        timestamp: mockStoredRow.created_at,
    ...
    assert(!tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(tamperedNonceMsg), bs58_1.default.decode(mockStoredRow.signature), bs58_1.default.decode(mockStoredRow.wallet_address)), 'Tampered persisted nonce invalidates re-verification');
    // 2. Tampered Domain
    const tamperedDomainMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: 'localhost',
        walletAddress: mockStoredRow.wallet_address,
        timestamp: mockStoredRow.created_at,
        nonce: mockStoredRow.nonce,
    ...
    assert(!tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(tamperedDomainMsg), bs58_1.default.decode(mockStoredRow.signature), bs58_1.default.decode(mockStoredRow.wallet_address)), 'Tampered persisted domain invalidates re-verification');
    // 3. Tampered Content
    const tamperedE2EContentMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: mockStoredRow.domain,
        walletAddress: mockStoredRow.wallet_address,
        timestamp: mockStoredRow.created_at,
        nonce: mockStoredRow.nonce,
    ...
    assert(!tweetnacl_1.default.sign.detached.verify(new TextEncoder().encode(tamperedE2EContentMsg), bs58_1.default.decode(mockStoredRow.signature), bs58_1.default.decode(mockStoredRow.wallet_address)), 'Tampered persisted content invalidates re-verification');
    // --- SUITE 7: Deterministic Reputation Engine & Off-Chain Achievement System ---
    console.log('\n► SUITE 7: Deterministic Reputation Engine & Off-Chain Achievement System');
    const suite7Keypair = tweetnacl_1.default.sign.keyPair();
    const suite7Wallet = bs58_1.default.encode(suite7Keypair.publicKey);
    const log1Timestamp = '2026-08-01T10:00:00.000Z';
    const log1Nonce = bs58_1.default.encode(tweetnacl_1.default.randomBytes(16));
    const log1Content = 'Wrote Solana Anchor Program with Rust';
    const log1Prompt = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
    ...
    const log1Sig = bs58_1.default.encode(tweetnacl_1.default.sign.detached(new TextEncoder().encode(log1Prompt), suite7Keypair.secretKey));
    const log2Timestamp = '2026-08-02T10:00:00.000Z';
    const log2Nonce = bs58_1.default.encode(tweetnacl_1.default.randomBytes(16));
    const log2Content = 'Integrated Metaplex Compressed NFTs';
    const log2Prompt = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: 'provn-sol.vercel.app',
        walletAddress: suite7Wallet,
    ...
    const log2Sig = bs58_1.default.encode(tweetnacl_1.default.sign.detached(new TextEncoder().encode(log2Prompt), suite7Keypair.secretKey));
    const testLogs = [
        {
            id: 1,
    ...
    const rep1 = (0, reputationEngine_1.calculateReputation)(suite7Wallet, testLogs);
    const rep2 = (0, reputationEngine_1.calculateReputation)(suite7Wallet, testLogs);
    ...
    const achievements = (0, achievements_1.evaluateAchievements)(testLogs, 7, 7);
    ...
    const eligibility = (0, cnftEligibility_1.checkCNFTEligibility)('FIRST_PROOF', rep1);
    assert(eligibility.eligible === true, 'checkCNFTEligibility evaluates eligible for earned achievement');
    const meta = (0, cnftEligibility_1.generateAchievementMetadata)(genesisAch, rep1);
    ...
    const minter = new cnftEligibility_1.LocalTestMinter();
    ...
    const isSdkValid = index_1.ProvnClient.verifyProofLocally({
        walletAddress,
        signature,
        nonce,
        timestamp,
    ...
    const isSdkTamperedInvalid = index_1.ProvnClient.verifyProofLocally({
        walletAddress,
        signature,
        nonce,
        timestamp,
    ...
    const themeKeys = Object.keys(cardThemes_1.CARD_THEMES);
    ...
    for (const key of themeKeys) {
        const t = cardThemes_1.CARD_THEMES[key];
    ...
    const fallbackTheme = (0, cardThemes_1.getCardTheme)('unknown_nonexistent_theme');
    ...
    const decodedPubkey = (0, canonicalMessage_1.decodeBase58)(validSolanaWallet);
    assert(decodedPubkey.length === 32, 'Valid Solana address decodes to exact 32-byte public key');
    let invalidBase58Thrown = false;
    try {
        (0, canonicalMessage_1.decodeBase58)('Invalid0OIlNonBase58Chars!!!');
    }
    catch {
    ...
    const { BUILDER_LEVELS, getBuilderLevel } = await Promise.resolve().then(() => __importStar(require('../app/lib/milestones')));
    ...
    const retryMsg = (0, canonicalMessage_1.buildCanonicalRetryMessage)({
    ...
    const suite9Keypair = tweetnacl_1.default.sign.keyPair();
    const suite9Wallet = bs58_1.default.encode(suite9Keypair.publicKey);
    const suite9Timestamp = new Date().toISOString();
    const suite9Nonce = bs58_1.default.encode(tweetnacl_1.default.randomBytes(16));
    ...
    const canonicalPrompt = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: suite9Domain,
    ...
    const validSigBytes = tweetnacl_1.default.sign.detached(new TextEncoder().encode(canonicalPrompt), suite9Keypair.secretKey);
    const testSignature = bs58_1.default.encode(validSigBytes);
    const validLogObj = {
        id: 101,
    ...
    assert((0, canonicalMessage_1.verifyLogCryptographically)(validLogObj) === true, 'verifyLogCryptographically returns true for valid authentic Ed25519 signature');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, content: 'Tampered work text' }) === false, 'verifyLogCryptographically returns false for tampered content');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, github_url: 'https://github.com/dren712/pow-logger/pull/100' }) === false, 'verifyLogCryptographically returns false for tampered GitHub link');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, github_url: 'https://evil.example/pr' }) === false, 'verifyLogCryptographically returns false for invalid non-github URL');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, evidence_url: 'http://insecure-http.com' }) === false, 'verifyLogCryptographically returns false for insecure non-https evidence URL');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, domain: 'evil.example' }) === false, 'verifyLogCryptographically returns false for tampered domain');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, nonce: 'non_base58_nonce_with_invalid_chars!' }) === false, 'verifyLogCryptographically returns false for non-Base58 nonce');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, nonce: ' ' + suite9Nonce }) === false, 'verifyLogCryptographically returns false for whitespace-padded nonce');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, nonce: null }) === false, 'verifyLogCryptographically returns false when nonce is missing');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, signature: null }) === false, 'verifyLogCryptographically returns false when signature is missing');
    assert((0, canonicalMessage_1.verifyLogCryptographically)({ ...validLogObj, signature: 'FakeSig1234567890123456789012345678901234567890123456789012345678901234' }) === false, 'verifyLogCryptographically returns false for invalid forged signature');
    ...
    const forgedLogs = [
        {
            ...
            nonce: bs58_1.default.encode(tweetnacl_1.default.randomBytes(16)),
    ...
    const mixedLogs = [validLogObj, ...forgedLogs];
    const pureReputation = (0, reputationEngine_1.calculateReputation)(suite9Wallet, mixedLogs);
    ...
    console.log('\n► SUITE 10: Policy Evaluation Engine & Proof Packet Generation');
    const { evaluateEligibility, STANDARD_POLICY_PRESETS } = await Promise.resolve().then(() => __importStar(require('../app/lib/policyEngine')));
    ...
    const generatedPacket = index_1.ProvnClient.generateProofPacket(samplePassport);
    ...
    console.log('\n► SUITE 11: Protocol V2 & Server-Issued Challenge Integrity');
    assert(canonicalMessage_1.CURRENT_PROTOCOL_VERSION === 2, 'Current protocol version is exported as 2');
    const testV2Keypair = tweetnacl_1.default.sign.keyPair();
    const testV2Wallet = bs58_1.default.encode(testV2Keypair.publicKey);
    ...
    const v2SubmitMsg = (0, canonicalMessage_1.buildCanonicalSubmitMessageV2)({
        domain: testV2Domain,
    ...
    const v2RetryMsg = (0, canonicalMessage_1.buildCanonicalRetryMessageV2)({
        domain: testV2Domain,
        walletAddress: testV2Wallet,
        logId: 42,
        timestamp: testV2Timestamp,
    ...
    const v2SigBytes = tweetnacl_1.default.sign.detached(v2MsgBytes, testV2Keypair.secretKey);
    const v2Signature = bs58_1.default.encode(v2SigBytes);
    ...
    assert((0, canonicalMessage_1.verifyLogCryptographically)(authenticV2Log) === true, 'verifyLogCryptographically authenticates valid Protocol V2 record');
    const tamperedV2ChallengeLog = {
        ...authenticV2Log,
        nonce: 'tampered-challenge-value-12345678',
    };
    assert((0, canonicalMessage_1.verifyLogCryptographically)(tamperedV2ChallengeLog) === false, 'verifyLogCryptographically strictly rejects tampered challenge in V2');
    const tamperedV2ContentLog = {
        ...authenticV2Log,
        content: 'Tampered content attempting unauthorized modification',
    };
    assert((0, canonicalMessage_1.verifyLogCryptographically)(tamperedV2ContentLog) === false, 'verifyLogCryptographically strictly rejects tampered content in V2');
    // Backward compatibility: V1 log verification
    const v1Keypair = tweetnacl_1.default.sign.keyPair();
    const v1Wallet = bs58_1.default.encode(v1Keypair.publicKey);
    const v1Timestamp = new Date().toISOString();
    const v1Nonce = '999999999999';
    const v1Content = 'V1 legacy log submission';
    const v1Msg = (0, canonicalMessage_1.buildCanonicalSubmitMessage)({
        domain: 'provn-sol.vercel.app',
    ...
    const v1Sig = bs58_1.default.encode(tweetnacl_1.default.sign.detached(new TextEncoder().encode(v1Msg), v1Keypair.secretKey));
    ...
    assert((0, canonicalMessage_1.verifyLogCryptographically)(authenticV1Log) === true, 'verifyLogCryptographically maintains backward compatibility for V1 logs');
    ...
    assert((0, canonicalMessage_1.verifyLogCryptographically)(tamperedV2DomainLog) === false, 'verifyLogCryptographically strictly rejects tampered domain in V2');
    // V2 GitHub URL tamper rejection
    const tamperedV2GithubLog = {
        ...authenticV2Log,
        github_url: 'https://github.com/evil/repo',
    };
    assert((0, canonicalMessage_1.verifyLogCryptographically)(tamperedV2GithubLog) === false, 'verifyLogCryptographically strictly rejects tampered GitHub URL in V2');
    // V2 Evidence URL tamper rejection
    const tamperedV2EvidenceLog = {
        ...authenticV2Log,
        evidence_url: 'https://evil.com/tampered',
    };
    assert((0, canonicalMessage_1.verifyLogCryptographically)(tamperedV2EvidenceLog) === false, 'verifyLogCryptographically strictly rejects tampered evidence URL in V2');
    ...
    assert((0, canonicalMessage_1.verifyLogCryptographically)(persistedV2Log) === true, 'verifyLogCryptographically verifies persisted V2 log with challenge in nonce column');
    ...
    const archiveMsg = (0, canonicalMessage_1.buildCanonicalArchiveMessage)({
        domain: 'provn-sol.vercel.app',
        walletAddress: testV2Wallet,
        logId: 42,
        challenge: testV2Challenge,
    ...
    const visMsg = (0, canonicalMessage_1.buildCanonicalVisibilityMessage)({
        domain: 'provn-sol.vercel.app',
        walletAddress: testV2Wallet,
        logId: 42,
        visibility: 'public',
